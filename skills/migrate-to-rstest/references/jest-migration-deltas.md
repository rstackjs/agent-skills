...
1. **Legacy file enumeration for phase (b) of SKILL principle 6.** Once Rstest is green, delete: `jest.config.*`, `jest.setup.*`, and any companion `jest.*.ts` (for example `jest.global-setup.ts`, `jest.global-teardown.ts`). Remove from `devDependencies` the ones that actually appear: `jest`, `ts-jest`, `@types/jest`, `jest-environment-jsdom`, `identity-obj-proxy`.

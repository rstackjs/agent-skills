...
2. Obtain the explicit Rsdoctor `dataFile`. If it is missing, identify the product and offer the minimal capture command: `RSTACK_CONTEXT=1 RSDOCTOR=true RSDOCTOR_OUTPUT=json rs build` for an application or `rs lib` for a library. Ask before installing, building, or capturing.

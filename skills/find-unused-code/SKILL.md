...
3. Call `product_roots` with `contextId`, `dataFile`, and `rootLimit: 20`, then report complete counts from `rootSummary` plus representative production and published-contract roots and graph issues. Request more roots only when the investigation needs them.
...
Optimizer retention is independent module evidence. A side-effect, CommonJS, or dynamic-import
bailout can explain why reached code was retained, but it is not a product root and does not by
itself disqualify an unreachable candidate.

...
4. When a context exists, call `product_roots` with the selected `contextId` and `dataFile`. Use context-bound claims only when `artifactBinding` is `exact`; report `mismatch` or `explicit-unverified` as artifact-only evidence.

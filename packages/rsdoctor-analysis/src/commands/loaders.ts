...
export function registerLoaderCommands(
  ...
  const loaderProgram = program
    .command('loaders')
    .description('Loader operations');
  ...
  loaderProgram
    ...
    .option(
      '--min-costs <minCosts>',
      'Minimum costs threshold (filter by minimum costs)',
    )
    .action(function (this: Command) {
      ...
      return execute(() =>
        getHotFiles(options.pageNumber, options.pageSize, options.minCosts),
      );
    });
  ...
  loaderProgram
    ...
    .option(
      ...
      'Minimum total costs threshold (filter by minimum total costs)',
    )
    .action(function (this: Command) {
      const options = this.opts<{
        pageNumber?: string;
        pageSize?: string;
        minTotalCosts?: string;
      }>();
      return execute(() =>
        getDirectories(
          options.pageNumber,
          options.pageSize,
          options.minTotalCosts,
        ),
      );
    });
}

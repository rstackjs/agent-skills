...
Daily AI eco-ci triage found that this PR appears to have caused the `<suite>` suite to fail in `rstack-ecosystem-ci`.
...
This attribution is based on <success-to-failure pivot | canary bisect | matching current and first-bad signatures>. Please take a look when you have time.
```

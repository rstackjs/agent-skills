...
4. Treat snapshot cleanup as a separate parity gate. A green Rstest run can still leave old Jest snapshot exports unchecked when test names or key separators change. After non-snapshot failures are fixed, run `rstest -u` (or the package's equivalent update script) for the exact migrated scope. Review the diff to confirm that removed entries are stale and that any new or changed bodies correspond to the same assertions; do not accept wholesale body changes just because the update command passes. Re-run normal `rstest` and require `unmatched: 0` and `unchecked: 0` when the installed reporter exposes those counters; otherwise compare the old and new snapshot key lists manually and explain any remaining stale entries.

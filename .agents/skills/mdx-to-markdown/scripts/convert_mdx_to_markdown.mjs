...
function normalizeLines(text, linkPrefix) {
  ...
  const emitFence = () => {
    const hasHighlight = fence.lines.some((line) =>
      CODE_HIGHLIGHT_MARKER.test(line),
    );
  ...
  for (const rawLine of lines) {
    ...
    const fenceWithTitle = line.match(
      /^```([A-Za-z0-9_-]+)\s+title="([^"]+)"$/,
    );
    if (fenceWithTitle) {
...
function parseArgs(argv) {
  ...
  return {
    ...
    output:
      positional[1] ??
      join(
        dirname(positional[0]),
        `${basename(positional[0], extname(positional[0]))}.md`,
      ),
    linkPrefix,
  };
}
...
try {
  ...
  writeFileSync(
    output,
    convert(text, linkPrefix ?? inferLinkPrefix(input)),
    'utf8',
  );

...
import { requireArg, parseNumber, parsePositiveInt } from '../utils/cli-utils';
...
export async function getChunkById(chunkIdInput: string | undefined): Promise<{ ok: boolean; data: unknown }> {
...
export async function findLargeChunks(): Promise<{ ok: boolean; data: { median: number; operator: number; minSizeMB: number; oversized: unknown[] }; description: string }> {
...
export function registerChunkCommands(program: Command, execute: CommandExecutor): void {

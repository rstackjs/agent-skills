...
macOS Docker `codspeed run -m simulation` caveat: the runner wraps Valgrind in `setarch --personality`, which Docker Desktop's Linux VM blocks even with `--cap-add=SYS_ADMIN`. The failure surfaces as `setarch: failed to set personality to aarch64: Operation not permitted` followed by `failed to execute valgrind`. The CodSpeed Valgrind fork itself has arm64 builds (verified with `valgrind-3.26.0.codspeed` arm64 `.deb`); only the runner's `setarch` step is blocked. On macOS, either export `MICRO_OPT_PLATFORM=linux/amd64` and rerun under Rosetta/QEMU, or skip `codspeed run` and invoke `valgrind --tool=callgrind` directly on the binary produced by `cargo codspeed build -m simulation`. Treat any prior baseline taken under a different platform or runner mode as invalid; collect a fresh baseline before comparing.
...
- `codspeed run -m simulation` does not work in macOS Docker under `linux/arm64`: the runner's `setarch --personality aarch64` step is blocked by Docker Desktop's Linux VM (even `--cap-add=SYS_ADMIN --security-opt apparmor=unconfined` does not unblock it), surfacing as `failed to execute valgrind`. The CodSpeed Valgrind fork itself supports arm64 (prebuilt `.deb` available since `3.24.0-0codspeed1`); the block is in the runner wrapper, not the architecture or the fork. Two fixes: (1) `export MICRO_OPT_PLATFORM=linux/amd64`, rebuild, and collect a fresh baseline; or (2) skip `codspeed run` entirely and call `valgrind --tool=callgrind` on the binary produced by `cargo codspeed build -m simulation`, accepting that ASLR is not disabled. Do not mix platforms or modes within one comparison.
